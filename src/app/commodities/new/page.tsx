...
import { createCommodity } from '@/app/actions/commodity';
      ...
      <div className="max-w-5xl mx-auto p-4 space-y-6">
        ...
        <header className="flex items-center justify-between py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-slate-900">Add Commodity Master</h1>
              <p className="text-slate-500 text-sm">Configure Brokerage Packagings & Quality Specs</p>
            </div>
          </div>
        </header>

        <form action={createCommodity} className="space-y-6">
          ...
          <div className="bg-white shadow-md border border-slate-200 rounded-xl overflow-hidden">
            ...
            <div className="p-6 grid md:grid-cols-12 gap-6 bg-white">
               <div className="md:col-span-6">
                 ...
                 <input required name="name" type="text" className="w-full rounded border-slate-300 p-2.5 text-sm bg-white border outline-none font-bold text-slate-800 focus:ring-2 focus:ring-indigo-500" placeholder="e.g. BAJRA CATTLEFEED" />
               </div>
               ...
               <div className="md:col-span-3">
                 ...
                 <select name="unit" className="w-full rounded border-slate-300 p-2.5 text-sm bg-white border outline-none focus:ring-2 focus:ring-indigo-500">
                   <option>KG</option>
                   <option>QTL</option>
                   <option>MT</option>
                   <option>BAG</option>
               ...
               <div className="md:col-span-8">
                 ...
                 <input name="description" type="text" className="w-full rounded border-slate-300 p-2.5 text-sm bg-white border outline-none focus:ring-2 focus:ring-indigo-500" />
               </div>
               <div className="md:col-span-2">
                 <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Short Name</label>
                 <input name="shortName" type="text" className="w-full rounded border-slate-300 p-2.5 text-sm bg-white border outline-none focus:ring-2 focus:ring-indigo-500 font-mono" placeholder="BAJRA" />
               </div>
               <div className="md:col-span-2">
                 <label className="block text-xs font-bold text-slate-600 uppercase mb-1">HSN Code</label>
                 <input name="hsnCode" type="text" className="w-full rounded border-slate-300 p-2.5 text-sm bg-white border outline-none focus:ring-2 focus:ring-indigo-500 font-mono" />
               </div>
            </div>
          </div>
                 ...
                 <tbody className="divide-y divide-slate-100">
                    ...
                    {packList.map((p, i) => (
                      <tr key={i} className="hover:bg-slate-50">
                        <td className="p-2"><input type="number" value={p.packingWeight} onChange={(e)=>updatePackRow(i, 'packingWeight', e.target.value)} className="w-full border border-slate-300 rounded p-1.5 focus:border-indigo-500 outline-none" placeholder="e.g. 50.00" /></td>
                        <td className="p-2">
                           <select value={p.packingType} onChange={(e)=>updatePackRow(i, 'packingType', e.target.value)} className="w-full border border-slate-300 rounded p-1.5 focus:border-indigo-500 outline-none bg-white">
                             <option>Katta</option><option>Bori</option><option>Box</option><option>Loose</option>
                           </select>
                        </td>
                        <td className="p-2"><input type="number" value={p.sellerBrokerageRate} onChange={(e)=>updatePackRow(i, 'sellerBrokerageRate', e.target.value)} className="w-full border border-slate-300 rounded p-1.5 focus:border-indigo-500 outline-none" placeholder="10.00" /></td>
                        <td className="p-2">
                           <select value={p.sellerBrokerageType} onChange={(e)=>updatePackRow(i, 'sellerBrokerageType', e.target.value)} className="w-full border border-slate-300 rounded p-1.5 focus:border-indigo-500 outline-none bg-white">
                             <option>PQtl</option><option>PBag</option><option>PMT</option><option>Percent</option>
                           </select>
                        </td>
                        <td className="p-2"><input type="number" value={p.buyerBrokerageRate} onChange={(e)=>updatePackRow(i, 'buyerBrokerageRate', e.target.value)} className="w-full border border-slate-300 rounded p-1.5 focus:border-indigo-500 outline-none" placeholder="9.00" /></td>
                        <td className="p-2">
                           <select value={p.buyerBrokerageType} onChange={(e)=>updatePackRow(i, 'buyerBrokerageType', e.target.value)} className="w-full border border-slate-300 rounded p-1.5 focus:border-indigo-500 outline-none bg-white">
                             <option>PQtl</option><option>PBag</option><option>PMT</option><option>Percent</option>
                           </select>
                        </td>
                 ...
                 <tbody className="divide-y divide-slate-100">
                    ...
                    {specList.map((s, i) => (
                      <tr key={i} className="hover:bg-slate-50">
                        ...
                        <td className="p-2"><input type="number" value={s.specValue} onChange={(e)=>updateSpecRow(i, 'specValue', e.target.value)} className="w-full border border-slate-300 rounded p-1.5 focus:border-indigo-500 outline-none" placeholder="12.00" /></td>
                        <td className="p-2">
                           <select value={s.minMax} onChange={(e)=>updateSpecRow(i, 'minMax', e.target.value)} className="w-full border border-slate-300 rounded p-1.5 focus:border-indigo-500 outline-none bg-white">
          ...
          <div className="pt-2 flex justify-end gap-4">
             ...
             <button type="submit" className="px-8 py-2.5 bg-indigo-600 text-white font-bold rounded-lg shadow hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-300 active:scale-95 transition-all flex items-center gap-2">
               <CheckCircle2 className="w-4 h-4" /> Save Commodity Master
             </button>
          </div>
